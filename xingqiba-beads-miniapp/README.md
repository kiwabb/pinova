...
- 工作台支持管理员登录后一键把生成的图纸导入云端图集：可选择现有图集或新建草稿图集、自动上传图纸图片、可同时补充封面，并记住上次导入目标
...
生成图纸后点击「导入图集」，使用管理后台账号登录即可把图纸直接写入云端图集（走 `adminService` 的 `importPattern`，事务追加并记录操作日志）。整个过程只上传生成的图纸图片，原始照片仍然留在本机。
