import { adjustContrast, deltaE2000, rgbToLab } from './color'
import { studioPalette } from './palette'
import type { ConvertRequest, ConvertResult, ImageMode, RGB, RasterImage } from './types'
...
function averageSample(image: RasterImage, x0: number, y0: number, x1: number, y1: number, mode: ImageMode): Sample {
  if (mode === 'pixel') {
...
export function convertImage({ image, settings }: ConvertRequest): ConvertResult {
  ...
  let cells = settings.dither
    ? mapWithDither(samples, width, height)
    : new Int16Array(samples.map((sample) => sample.alpha < 48 ? -1 : nearestPaletteIndex(sample)))
